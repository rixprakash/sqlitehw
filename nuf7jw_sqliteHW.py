...
#bonus task
#visual results in bar chart
dfcolumns = ['subject', 'student_name', 'highest_grade']
highest_grades_df = pd.DataFrame(highestgradestudents, columns=dfcolumns)
#was struggling on how to make the color based on students but found resources online that explained the process
unique_students = highest_grades_df['student_name'].unique()
colors = plt.cm.get_cmap('tab10', len(unique_students)) 
colormap = {student: colors(i) for i, student in enumerate(unique_students)}
plt.figure(figsize=(10, 6))
plt.bar(highest_grades_df['subject'], highest_grades_df['highest_grade'], color= highest_grades_df['student_name'].map(colormap))
plt.xlabel('Subject')
